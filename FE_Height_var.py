...
def find_nonrocket_in_JPR_image():
  ...
  for x in range(200, 580, int(tpl_w/2)):
    ...
    for y in range(sky, water, int(tpl_h/2)):
       ...
       if y + tpl_h >= img_h:
         ...
         h = tpl_h
       print("SEND: Here we start at top")
       temp = JPR_examine_area_for_nonrocket(x, y, w, h, sky, water)
       if temp is not None:
         for i in temp:
           rect_list.append(i)
  #print("rect_list for image: {}".format(rect_list))
  ...
  for rect in rect_list:
      ...
      if name == 'top_edge':
          color_b = 0; color_g = 200; color_r = 50;
      if name == 'bottom_edge':
          color_b = 0; color_g = 50; color_r = 200;
      if name == 'bird':
         color_b = 0; color_g = 0; color_r = 255
      if name == 'monster':
         color_b = 0; color_g = 255; color_r = 255
...
def JPR_examine_area_for_nonrocket(x0, y0, w, h, top_y_image, bottom_y_image ):
  print("TOP: starting {}+{} {}x{} top {} bottom {}".format(x0, y0, w, h, top_y_image, bottom_y_image))
  
  objects = ['barrier', 'bird', 'top_edge', 'bottom_edge'] 
  count = {}
  count_subsq = {}
  ...
  for x   in range(x0, x0 + w - 2, 1):
     for y in range(y0, y0 + h - 2, 1):
       ...
       #color_values = [red, green, blue, ratio_rg,ratio_rb,ratio_gb, intensity]
       
        ##may need seperate function to determine if barrier or swirl. From this code and the screenshot the swirls arnt idenified.
        ## Have the general swirl and split apart 
       

       if (ratio_rb > 3 and   
           red > 200 and green > 100 and blue < 50): #Finds bird
         count['bird'] = count['bird'] + 1
       if (ratio_rb > 5 and 
           red > 120 and green > 50 and blue < 50):
         count['bird'] = count['bird'] + 1

       if (y + 60 >= bottom_y_image) or (60-y >= 0): #Numbers to be optimized, finds barrier or bird
            
          print(f"This is the total amount for the counter {counter_correct_range}")
          if counter_correct_range >=200: ## I think this is how it works, assuming 4 total divisions
              counter_correct_range = 0
              
          elif counter_correct_range <200 and counter_correct_range >=100:
              if(60-y <= 0):
                counter_correct_range = 0
                count['top_edge'] += 1
              elif(y+50 >= bottom_y_image):
                  counter_correct_range = 0
                  count['bottom_edge'] += 1
              else:
                  counter_correct_range = 0
                  print("Out of tolerance for edge")
          else:
              counter_correct_range = 0
              print("Neither an edge or swirl")



            #result_of_edge = barrier_or_edge(x, y, w, h, top_y_image, bottom_y_image)
            #print(result_of_edge)
            if result_of_edge != '':
                count[result_of_edge] += 1
            
       else: ##Rational that if there is a barrier it could also be a swirl, even thought the color analysis would be the same
...
##         count['barrier'] = count['barrier'] + 1
  ...
  print("GOOD NEWS: We made it out of the loop")
  result = []
  thresholds = {'barrier': 0.8, 'bird': 0.7, 'top_edge': 0.6, 'bottom_edge': 0.6} #Edge numbers to be optimized 
  for object in ('barrier', 'bird', 'top_edge', 'bottom_edge'):
...
def barrier_or_edge(x0, y0, w, h, top_y_image, bottom_y_image):

    ##Possible to split the segment further into four pieces and then if three pieces are the same color then its a swirl, all four for barrier?
    counter_correct_range = 0
    print(f"x {x0}, y {y0}") #, top y {top_y_image}, bottom y {bottom_y_image}")
    y_range_max = 0 ##since scanning from bottom to top.
    #x_range_max = 0 
    if (bottom_y_image > (y0+h-2)):
      y_range_max = y0+h-2
    else:
      y_range_max = bottom_y_image

      
    for x   in range(x0, x0 + w-2, 1): ##I know this is wrong
     
      for y in range(y0, y_range_max, 1):
           divisions_per_dimension = 2 # ask how the heck this works. 
           template_size                = 20  # we have to fix this here because w/h are reduced at the edges of images
           pixels_per_division         = int( template_size / divisions_per_dimension )
           x1 = int( (x - x0) / pixels_per_division)
           y1 = int( (y - y0) / pixels_per_division)   # assume we are using a square, otherwise compute per dimension
